...
struct CustomDeviceQt : public CustomDeviceInterface {

    CustomDeviceQt() = default;
    ...
    virtual bool init(const QString& backend, const QString& iface) override
    {
        ...
        if (!_device) {
            cds_error("Failed to create customdevice");
    ...
    virtual bool writeFrame(const QCanBusFrame& frame) override
    {
        if (_device) {
            ...
            cds_error("customdevice is null. Call init first!");
            throw std::runtime_error("customdevice is null. Call init first!");
        }
