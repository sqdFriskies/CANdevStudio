...
class CustomDevicePrivate;
class QCanBusFrame;

/**
*   @brief The class provides abstraction layer for CAN BUS hardware
*/
class CustomDevice : public QObject, public ComponentInterface {
    Q_OBJECT
    Q_DECLARE_PRIVATE(CustomDevice)

public:
    explicit CustomDevice();
    explicit CustomDevice(CustomDeviceCtx&& ctx);
    ~CustomDevice();
...
private:
    QScopedPointer<CustomDevicePrivate> d_ptr;
};
