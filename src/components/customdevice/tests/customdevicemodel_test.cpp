...
TEST_CASE("Test basic functionality", "[customdevice]")
{
    using namespace fakeit;
    CustomDeviceModel canDeviceModel;
    REQUIRE(canDeviceModel.caption() == "CustomDevice");
    REQUIRE(canDeviceModel.name() == "CustomDevice");
    REQUIRE(canDeviceModel.resizable() == false);
    REQUIRE(dynamic_cast<CustomDeviceModel*>(canDeviceModel.clone().get()) != nullptr);
    REQUIRE(dynamic_cast<QLabel*>(canDeviceModel.embeddedWidget()) != nullptr);
}

TEST_CASE("Port information", "[customdevice]")
{
    CustomDeviceModel canDeviceModel;
...
TEST_CASE("Calling frameReceived emits dataUpdated and outData returns that frame", "[customdevice]")
{
    CustomDeviceModel canDeviceModel;
    QCanBusFrame testFrame;
    testFrame.setFrameId(123);
    QSignalSpy dataUpdatedSpy(&canDeviceModel, &CustomDeviceModel::dataUpdated);
...
TEST_CASE("Calling frameSent emits dataUpdated and outData returns that frame", "[customdevice]")
{
    CustomDeviceModel canDeviceModel;
    QCanBusFrame testFrame;
    testFrame.setFrameId(123);
...
TEST_CASE("Calling setInData will result in sendFrame being emitted", "[customdevice]")
{
    CustomDeviceModel canDeviceModel;
    QCanBusFrame testFrame;
    testFrame.setFrameId(123);
    auto canDeviceDataIn = std::make_shared<CanRawData>(testFrame);
    QSignalSpy sendFrameSpy(&canDeviceModel, &CustomDeviceModel::sendFrame);
...
TEST_CASE("Test save configuration", "[customdevice]")
{
    CustomDeviceModel canDeviceModel;
...
TEST_CASE("Getters", "[customdevice]")
{
    CustomDeviceModel cdModel;

    REQUIRE(cdModel.painterDelegate() != nullptr);
    REQUIRE(cdModel.hasSeparateThread() == true);
}

TEST_CASE("Tx queue full test", "[customdevice]")
{
    CustomDeviceModel cdModel;
    QCanBusFrame frame;
...
TEST_CASE("Rx queue full test", "[customdevice]")
{
    CustomDeviceModel cdModel;
    QCanBusFrame frame;
