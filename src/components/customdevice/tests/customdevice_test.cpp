...
#include <customdeviceinterface.h>
#include <catch.hpp>
#include <context.h>
#include <fakeit.hpp>
#include <log.h>
...
void setupBackendInterface(CustomDevice& canDevice)
{
    QWidget qo;
...
TEST_CASE("Initialization failed", "[customdevice]")
{
    CustomDevice canDevice;
    QCanBusFrame frame;
...
TEST_CASE("Initialization succeeded", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
    ...
    CustomDevice canDevice{ CustomDeviceCtx(&deviceMock.get()) };
    setupBackendInterface(canDevice);
...
TEST_CASE("Start failed", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Start failed - could not connect to device", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Start succeeded", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Stop uninitialized", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Stop initialized", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Config changed", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("writeFrame results in error", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("sendFrame, writeframe returns true, no signal emitted", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
    ...
    CustomDevice canDevice{ CustomDeviceCtx(&deviceMock.get()) };
    QSignalSpy frameSentSpy(&canDevice, &CustomDevice::frameSent);

    canDevice.sendFrame(testFrame);
    REQUIRE(frameSentSpy.count() == 0);
}

TEST_CASE("sendFrame, no device available, frameSent is not emitted", "[customdevice]")
{
    using namespace fakeit;
...
TEST_CASE("sendFrame defers FrameSent until backend emits frameWritten", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;

    const auto frame = QCanBusFrame{ 0x12345678, QByteArray{ "\x50\x30\10" } };
    CustomDeviceInterface::framesWritten_t writtenCbk;
...
TEST_CASE("Emits all available frames when notified by backend", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
    ...
    CustomDeviceInterface::framesReceived_t receivedCbk;
    ...
    CustomDevice canDevice{ CustomDeviceCtx(&deviceMock.get()) };
    QSignalSpy frameReceivedSpy(&canDevice, &CustomDevice::frameReceived);
    setupBackendInterface(canDevice);
    REQUIRE(canDevice.init() == true);
...
TEST_CASE("WriteError causes emitting frameSent with frameSent=false", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;

    const auto frame = QCanBusFrame{ 0x12345678, QByteArray{ "\x50\x30\10" } };
    CustomDeviceInterface::errorOccurred_t errorCbk;
...
TEST_CASE("read configuration to json format", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
    Fake(Dtor(deviceMock));
...
TEST_CASE("setConfig using JSON read with QObject", "[customdevice]")
{
    using namespace fakeit;
...
TEST_CASE("Stubbed methods", "[customdevice]")
{
    using namespace fakeit;
    Mock<CustomDeviceInterface> deviceMock;
    Fake(Dtor(deviceMock));
...
void prepareConfigTestMock(fakeit::Mock<CustomDeviceInterface>& deviceMock, std::vector<std::pair<int, QVariant>>& ver)
{
    using namespace fakeit;
...
void testConfig(fakeit::Mock<CustomDeviceInterface>& deviceMock, CustomDevice& canDevice, const QString& configStr)
{
    QWidget qo;
...
TEST_CASE("Config parameter - invalid format and unsupported", "[customdevice]")
{
    using namespace fakeit;
...
TEST_CASE("Config parameter - LoopbackKey", "[customdevice]")
{
    using namespace fakeit;
    std::vector<std::pair<int, QVariant>> v;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Config parameter - ReceiveOwnKey", "[customdevice]")
{
    using namespace fakeit;
    std::vector<std::pair<int, QVariant>> v;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Config parameter - CanFdKey", "[customdevice]")
{
    using namespace fakeit;
    std::vector<std::pair<int, QVariant>> v;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Config parameter - BitRateKey", "[customdevice]")
{
    using namespace fakeit;
    std::vector<std::pair<int, QVariant>> v;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Config parameter - UserKey", "[customdevice]")
{
    using namespace fakeit;
    std::vector<std::pair<int, QVariant>> v;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Config parameter - DataBitRateKey", "[customdevice]")
{
    using namespace fakeit;
    std::vector<std::pair<int, QVariant>> v;
    Mock<CustomDeviceInterface> deviceMock;
...
TEST_CASE("Config parameter - multiple keys", "[customdevice]")
{
    using namespace fakeit;
    std::vector<std::pair<int, QVariant>> v;
    Mock<CustomDeviceInterface> deviceMock;
